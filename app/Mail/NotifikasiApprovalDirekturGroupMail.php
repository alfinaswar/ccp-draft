...
class NotifikasiApprovalDirekturGroupMail extends Mailable
{
    ...
    {
        $NamaPerusahaan = $this->rekomendasi->getPerusahaan->NamaLengkap;
        $rekomendasiDetail = $this->rekomendasi->getRekomendasi
            ->flatMap(function ($rekomendasi) {
                return $rekomendasi->getRekomedasiDetail ?? collect();
            })
            ->where('Rekomendasi', 1);
        $NamaBarang = $rekomendasiDetail->map(function ($detail) {
        ...
        $NamaBarangStr = ($NamaBarang && $NamaBarang->count())
            ? $NamaBarang->pluck('Nama')->filter()->join(', ')
            : '-';

        return $this->subject('Approval Pengajuan - ' . $NamaBarangStr . ' - ' . $NamaPerusahaan)
            ->view('emails.notifikasi-approval-direktur-group')
            ->with($data)
            ->attachData($pdf->output(), 'Lampiran-Pengajuan.pdf', [
